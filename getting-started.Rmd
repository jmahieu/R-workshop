...
The packages we will use to import, clean, and visualize data are all part of the so-called **tidyverse**. The packages in the tidyverse share a common philosophy of data and `R` programming, and are designed to work together. You can install the complete tidyverse with a single line of code:
...
As an example, we will use a dataset called "mtcars" with information about different car types. You can download the data in csv format [here](https://raw.githubusercontent.com/jmahieu/R-workshop/main/data/mtcars.csv) (Right click + "Save as"). Make sure that you save it as a csv file (with .csv as extension) and not as a text file (.txt)! Opening the file in Excel would look like this: 
